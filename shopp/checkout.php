...
<big>
	<a href="<?php shopp('cart','url'); ?>" class="green">< Revenir au panier</a>
</big>
...
<?php if (shopp('cart','hasitems')): ?>
	<!-- checkout function -->
	<?php
		// insere une balise script qui pose probleme avec le chargement ajax ?
		$temp = shopp('checkout','function', 'return=1');

		//var_dump($temp);

		echo str_replace('//','', strip_tags($temp,'<input>') );
	?>
	<!-- fin checkout function -->
	<ul class="checkout">
		...
		<?php if (shopp('cart','needs-shipped')): ?>
			<div class="inline"><?php shopp('checkout','same-shipping-address','label=Même adresse de livraison'); ?></div>
			</li>
	...
	<p class="submit"><?php shopp('checkout','submit','value=Envoyer la commande&class=green'); ?></p>
